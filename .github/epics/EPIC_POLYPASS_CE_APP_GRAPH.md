...
- [ ] **App graph**: Implement `polypass_app_graph.fl` — 12 ModuleNode definitions, `make_polypass_module` helper, `polypass_app_graph_register`, intra-graph REQUIRES edges
- [ ] **Bridge edges**: Implement `polypass_register_bridge_edges` — PolyOAuth SSO autofill bridge, PolyAuthenticator TOTP/FIDO2 bridge
- [ ] **CE meaning domains**: Implement `polypass_meaning.fl` — `security/vault_health`, `security/autofill_accuracy`, `security/sharing_patterns`
- [ ] **Noise filter**: Implement `polypass_noise_filter` — suppress autofill cache misses, heartbeats, sync ACKs, TOTP ticks, metering increments
- [ ] **SME panels**: Implement vault security posture panel + credential sharing governance panel
- [ ] **CE orchestrator**: Implement `polypass_register_ce` — register all domains, noise rules, and panels
...
1. `polypass_app_graph_register` adds exactly 12 modules with correct aperture partitions and SLA tiers
